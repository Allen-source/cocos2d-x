...
void SpriteLoader::onHandlePropTypeBlendFunc(Node * pNode, Node * pParent, const char * pPropertyName, BlendFunc pBlendFunc, CCBReader * ccbReader) {
    if(strcmp(pPropertyName, PROPERTY_BLENDFUNC) == 0) {
        ...
        Texture2D* pTexture = pSprite->getTexture();
        if (pTexture && pTexture->hasPremultipliedAlpha())
            pBlendFunc.src = BlendFunc::ALPHA_PREMULTIPLIED.src;
        else
            pBlendFunc.src = BlendFunc::ALPHA_NON_PREMULTIPLIED.src;
