...
ActionInterval* CCBAnimationManager::getAction(CCBKeyframe *pKeyframe0, CCBKeyframe *pKeyframe1, const std::string& propName, Node *pNode)
{
    ...
    else if (propName == "particleActive")
    {
        
        bool active = pKeyframe1->getValue().asBool();
        auto delay = DelayTime::create(duration);
        auto call = CallFunc::create([active, pNode](){
            if (active) {
                static_cast<ParticleSystem*>(pNode)->resetSystem();
            } else {
                static_cast<ParticleSystem*>(pNode)->stopSystem();
            }
        });
        
        return Sequence::create(delay, call, NULL);
    }
    else 
    {
...
std::vector<std::string> CCBAnimationManager::getSequenceNames()
{
    std::vector<std::string> aNames;
    for (auto seq : _sequences)
    {
        aNames.push_back(seq->getName());
    }
    return aNames;
}
// Custom actions
...
void CCBSoundEffect::update(float /*time*/)
{
    ...
    if (s_pCustomCallback) {
        s_pCustomCallback->PlaySoundFile(_soundFile);
    } else {
