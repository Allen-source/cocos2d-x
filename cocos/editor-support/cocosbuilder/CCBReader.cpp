...
Node* CCBReader::readNodeGraphFromFile(const char *pCCBFileName, Ref *pOwner, const Size &parentSize)
{
    ...
    if (!dataPtr->isNull())
    {
        ...
        //CCLOG("ccb %s rc = %d", strCCBFileName.c_str(), ret->getReferenceCount());
        return ret;
    }
    else
    {
...
#else
    if (!memberVarAssignmentName.empty() && memberVarAssignmentType != TargetType::NONE)
    {
        if(!_jsControlled)
        {
            Ref*  target = nullptr;
        ...
        {
            if (pParent) // Added By YCH : not add root node
            {
                if(memberVarAssignmentType == TargetType::DOCUMENT_ROOT)
                {
...
void CCBReader::releaseOneFileDataCache(const std::string& filename) {
    fileDataCache.erase(filename);
}
