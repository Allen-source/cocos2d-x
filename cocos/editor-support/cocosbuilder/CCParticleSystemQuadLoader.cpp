...
#define PROPERTY_PARTICLE_ACTIVE "particleActive"
...
void ParticleSystemQuadLoader::onHandlePropTypeIntegerLabeled(Node * pNode, Node * pParent, const char * pPropertyName, int pIntegerLabeled, CCBReader * ccbReader) {
    ...
    else if (strcmp(pPropertyName, PROPERTY_POSITION_TYPE) == 0)
    {
        ((ParticleSystem *)pNode)->setPositionType((ParticleSystem::PositionType)pIntegerLabeled);
    }
    else
    {
...
void ParticleSystemQuadLoader::onHandlePropTypeCheck(cocos2d::Node * pNode, cocos2d::Node * pParent, const char* pPropertyName, bool pCheck, CCBReader * ccbReader){
    if (strcmp(pPropertyName, PROPERTY_PARTICLE_ACTIVE) == 0) {
