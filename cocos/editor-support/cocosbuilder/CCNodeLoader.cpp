...
// #endif
        ...
        switch(type) 
        {
            ...
            case CCBReader::PropertyType::FONT_TTF:
            {
                ...
                if(fontTTF.find(".ttf") != std::string::npos)
                {
...
SpriteFrame * NodeLoader::parsePropTypeSpriteFrame(Node * pNode, Node * /*pParent*/, CCBReader * ccbReader, const char *pPropertyName)
{
    ...
    if(Director::getInstance()->getContentScaleFactor() == 2) {
        
        replace(spriteSheet, "res/1x", "res/2x");
        replace(spriteFile, "res/1x", "res/2x");
    }
    ...
    if (!spriteFile.empty())
    {
        ...
        if (spriteFrame && ccbReader->getAnimatedProperties()->find(pPropertyName) != ccbReader->getAnimatedProperties()->end())
        {
            ccbReader->getAnimationManager()->setObject(spriteFrame, pNode, pPropertyName);
        }
    }
    if (!spriteFrame) {
        CCLOG("CCBERROR: parsePropTypeSpriteFrame %s %s",spriteSheet.c_str(), spriteFile.c_str());
    }
    return spriteFrame;
}
...
Texture2D * NodeLoader::parsePropTypeTexture(Node * /*pNode*/, Node * /*pParent*/, CCBReader * ccbReader) {
    ...
    if (!spriteFile.empty())
    {
        tex = Director::getInstance()->getTextureCache()->addImage(spriteFile);
    }
    if (!tex) {
        CCLOG("CCBERROR: parsePropTypeTexture %s", spriteFile.c_str());
    }
    return tex;
}
...
{
    string result = ccbReader->readCachedString();
    
    if(Director::getInstance()->getContentScaleFactor() == 2) {
        
        replace(result, "res/1x", "res/2x");
    }
...
Node * NodeLoader::parsePropTypeCCBFile(Node * /*pNode*/, Node * pParent, CCBReader * pCCBReader) {
    ...
    reader->m_ccbFileName = ccbFileName;
...
//     reader->_ownerCallbackNodes->retain();
    ...
    if (reader->isJSControlled() && pCCBReader->isJSControlled() && nullptr == reader->_owner)
    {
        ...
        auto& ownerCallbackControlEvents = reader->getOwnerCallbackControlEvents();
        if (!ownerCallbackNames.empty() && !ownerCallbackNodes.empty())
        {
            ...
            for (ssize_t i = 0 ; i < nCount; i++)
            {
                ...
                pCCBReader->addOwnerCallbackControlEvents((Control::EventType)ownerCallbackControlEvents.at(i).asInt());
            }
        }
        //set variables
    ...
    if(ccbFileNode != nullptr){
        for(auto it=reader->getLoadedSpriteSheet().begin(); it!=reader->getLoadedSpriteSheet().end(); ++it){
            ccbFileNode->pushPlist(*it);
        }
    }
    reader->release();
    return ccbFileNode;
